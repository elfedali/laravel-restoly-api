<?php

namespace App\Policies;

use App\Models\User;
use App\Models\Restaurant;
use Illuminate\Auth\Access\HandlesAuthorization;

class RestaurantPolicy
{
    use HandlesAuthorization;

    /**
     * Determine whether the restaurant can view any models.
     */
    public function viewAny(User $user): bool
    {
        return true;
    }

    /**
     * Determine whether the restaurant can view the model.
     */
    public function view(User $user, Restaurant $model): bool
    {
        return true;
    }

    /**
     * Determine whether the restaurant can create models.
     */
    public function create(User $user): bool
    {
        return true;
    }

    /**
     * Determine whether the restaurant can update the model.
     */
    public function update(User $user, Restaurant $model): bool
    {
        return true;
    }

    /**
     * Determine whether the restaurant can delete the model.
     */
    public function delete(User $user, Restaurant $model): bool
    {
        return true;
    }

    /**
     * Determine whether the user can delete multiple instances of the model.
     */
    public function deleteAny(User $user): bool
    {
        return true;
    }

    /**
     * Determine whether the restaurant can restore the model.
     */
    public function restore(User $user, Restaurant $model): bool
    {
        return false;
    }

    /**
     * Determine whether the restaurant can permanently delete the model.
     */
    public function forceDelete(User $user, Restaurant $model): bool
    {
        return false;
    }
}
